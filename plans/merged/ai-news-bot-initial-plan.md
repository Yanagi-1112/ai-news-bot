...
- LLM: **OpenAI互換の単一クライアント実装＋.envで差し替え**（2026-09-07やなぎさん指示。
  solレビューの「1プロバイダ固定」は覆し、コード経路は1本のままbase_url/model/keyだけ差し替え可能にする折衷）。
  `.env.example` にプリセット3種（OpenAI / Gemini互換エンドポイント / DeepSeek）を記載し、
  モデル選定はうしくんさんに委ねる（候補: gpt-5.6-sol・Astra系・gemini-3.8）。テストはLLMスタブで実施
...
## 決定事項（2026-09-07 やなぎさん回答）

- LLM: 差し替え可能実装にする（上記アーキテクチャ参照）。モデル選定はうしくんさんに委任
  ※音声入力の「節くんさん」は「うしくんさん」と解釈（仮定。違ったら訂正を受ける）
- 定時投稿: 朝8:00 JST で確定
- 投稿フォーマット: シンプルなMarkdownテキスト。トップ1本はURLプレビューを活かし、
  次点は `<URL>` でプレビュー抑制（Fable提案・仮採用）
- フック: 自動しきい値判定のみ。手動トリガーは作らない

## 未解決の懸念

- mainへのPR必須化までやるか（2人運用なので、v1はforce-push禁止＋tag運用のみに留める想定）
- live smoke・E2Eに使う実APIキー（ローカルで利用可能なキーを実装後に確認）
...
- B5完了: テストWebhook（やなぎさん個人サーバー #一般）への実配信E2E成功（速報1件＋ダイジェスト1件、
  message ID取得・状態遷移確認・uncertain 0件）。READMEデモ画像は実配信内容をもとに生成して添付
- v0.1.0タグ発行。フェーズC（うしくんさんによるKAGOYAデプロイ）待ち
